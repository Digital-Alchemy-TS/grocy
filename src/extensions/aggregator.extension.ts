import { CronExpression, eachLimit, TServiceParams } from "@digital-alchemy/core";
...
export function Aggregator({ scheduler, logger, grocy, event, config, lifecycle }: TServiceParams) {
  ...
  const aggregator = {
    ...
    async buildBatteryCache(): Promise<void> {
      ...
      await eachLimit(battery, config.grocy.USERFIELDS_FETCH_RATE, async battery => {
        const userfields = await grocy.object.listObjectUserFields<GrocyBatteryUserfields>({
    ...
    async buildChoresCache(): Promise<void> {
      ...
      await eachLimit(chores, config.grocy.USERFIELDS_FETCH_RATE, async chore => {
        const data = (await grocy.chores.getChoreObject(chore.id)) as GrocyObjectChoreDetail;
    ...
    async buildTaskCache(): Promise<void> {
      ...
      await eachLimit(tasks, config.grocy.USERFIELDS_FETCH_RATE, async task => {
        const userfields = await grocy.object.listObjectUserFields<GrocyTaskUserfields>({
          id: task.id.toString(),
          type: "tasks",
        });
        cache.add({
